class AselvanScripts < Formula
  desc "aselvan.scripts --- a random collection of utility scripts"
  ...
  caveats do
    <<~EOS
      To add #{name} to your PATH, add the following line to your ~/.bashrc:
        export PATH="$PATH:#{opt_bin}/utils:#{opt_bin}/security:#{opt_bin}/tools:#{opt_bin}/macos:#{opt_bin}/firewal"

      Or, if you prefer, you can use the following command:
        echo 'export PATH="$PATH:#{opt_bin}/utils:#{opt_bin}/security:#{opt_bin}/tools:#{opt_bin}/macos:#{opt_bin}/firewal"' >> ~/.bashrc
    EOS
  end
